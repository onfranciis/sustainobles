const Six = () => {
  return (
    <div className="pb-24 px-16 -md:px-12 -sm:px-8 -xs:px-5 bg-white flex justify-center w-full ">
      <div className="bg-[url('/landing-two.png')] bg-no-repeat bg-center bg-cover p-8 bg-fixed -md:p-4 pr-[73px] pb-12 rounded-2xl w-full">
        <p className="mb-28 -md:mb-24 -sm:mb-20 text-[64px] -lg:text-[52px] -md:text-[40px] font-bold">
          Events
        </p>

        <p className="-sm:mb-2 mb-3 text-[40px] -lg:text-4xl -md:text-2xl leading-normal opacity-80">
          Medical outreach
        </p>

        <p className="text-[40px] -lg:text-4xl -md:text-2xl leading-normal opacity-80">
          Football
        </p>
      </div>
    </div>
  );
};

export default Six;
